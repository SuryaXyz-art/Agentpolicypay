...
import { receiptExplorerUrl } from "@/lib/network";

export default function ReceiptCard({ receipt, onView, onExport }: { receipt: Receipt; onView?: () => void; onExport?: () => void }) {
  const transactionHash = receipt.txHash ?? receipt.transactionHash;
  const explorerUrl = receiptExplorerUrl(receipt.chainId, transactionHash);
  ...
  return (
    <div className="card stack">
      ...
      <div className="grid gap-2 rounded-md border border-white/10 bg-night/60 p-3">
        ...
        {receipt.intentHash && <p className="code text-slate-500">Intent: {receipt.intentHash}</p>}
        {receipt.storageRoot && <p className="code text-slate-500">Storage root: {receipt.storageRoot}</p>}
      ...
      <div className="actions">
        ...
        {explorerUrl && <a className="button secondary" href={explorerUrl} target="_blank" rel="noreferrer">Explorer <ExternalLink size={16} /></a>}
      </div>
    </div>
  );
}
