...
import { AlertTriangle, Archive, Bot, DatabaseZap, ReceiptText, ServerCog, ShieldCheck, WalletCards } from "lucide-react";
  ...
  return (
    <main className="page">
      <section className="section">
        ...
        <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between"><div className="stack"><div className="eyebrow">Dashboard</div><h1 className="max-w-4xl text-4xl font-black leading-tight text-white sm:text-5xl">Apolo Mind control center</h1><p className="lede">Set rules once. Let agents pay safely.</p></div><div className="actions"><button className="button secondary" onClick={loadDemoData}><DatabaseZap size={18} /> Load offline sample</button><Link className="button" href="/request-payment">Request guarded payment <WalletCards size={18} /></Link><Link className="button secondary" href="/mind-vault">Open Mind Vault <Archive size={18} /></Link></div></div>
        <div className="grid gap-5 md:grid-cols-2 xl:grid-cols-5"><StatCard label="Active policies" value={policy ? 1 : 0} detail="User spending policies currently enabled." icon={ShieldCheck} /><StatCard label="Approved agents" value={agents.filter((agent) => agent.approved).length} detail="Autonomous wallets trusted to request spend." icon={Bot} tone="violet" /><StatCard label="Allowed services" value={allowedServices.length} detail="Receiver addresses approved for payments." icon={ServerCog} tone="blue" /><StatCard label="Recorded spend" value={`${totalSpend} 0G`} detail="Local display only; the guarded vault is authoritative." icon={ReceiptText} /><StatCard label="Blocked attempts" value={blockedAttempts.length} detail="Requests stopped by policy checks." icon={AlertTriangle} tone="red" /></div>
