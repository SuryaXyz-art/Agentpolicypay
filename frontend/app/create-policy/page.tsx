...
function validateForm(form: FormState) {
  ...
  if (!Number.isFinite(approvalThreshold) || approvalThreshold <= 0) errors.approvalThreshold = "Approval threshold must be greater than 0.";
  ...
  if (Number.isFinite(approvalThreshold) && Number.isFinite(maxPerTx) && approvalThreshold > maxPerTx) {
    errors.approvalThreshold = "Approval threshold cannot exceed max spend per transaction.";
  }
...
export default function CreatePolicyPage() {
  ...
  const contractReady = isConnected && chainId === ACTIVE_CHAIN_ID && hasRegistryConfig();
  ...
  function saveOnChainPolicy(policyId: string, txHash?: string) {
  ...
  async function submitPolicy(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      if (!contractReady || !contractAddresses.registry || !publicClient) {
        throw new Error(`Live mode requires a connected wallet on ${ACTIVE_NETWORK_NAME} and a current Wave 3 registry address.`);
      }
      ...
      if (contractReady && contractAddresses.registry && publicClient) {
        ...
        if (receipt.status !== "success") throw new Error("Policy creation reverted.");
        const logs = parseEventLogs({
          abi: agentPolicyRegistryAbi,
          eventName: "PolicyCreated",
          logs: receipt.logs.filter(log => log.address.toLowerCase() === contractAddresses.registry!.toLowerCase())
        });
        const policyId = logs[0]?.args.policyId?.toString();
        if (!policyId) throw new Error("Confirmed transaction has no matching PolicyCreated event.");
        saveOnChainPolicy(policyId, hash);
        ...
        <div className="grid cols-2">
          <form className="card form" onSubmit={submitPolicy} noValidate>
            ...
            <p className="text-sm leading-6 text-slate-400">
              {contractReady
                ? `Configured Wave 3 registry on ${ACTIVE_NETWORK_NAME}. This will submit a transaction.`
                : `Live mode requires a wallet on ${ACTIVE_NETWORK_NAME} and a configured Wave 3 registry.`}
            </p>
          </form>
          ...
          <div className="stack">
            ...
            {success && (
              <div className="card stack border-emerald-400/25 bg-emerald-400/10">
                ...
                <p className="text-sm text-slate-300">Created through the configured Wave 3 AgentPolicyRegistry.</p>
