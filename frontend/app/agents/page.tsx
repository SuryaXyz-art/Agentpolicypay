...
import { defaultAgents, defaultServices, type Agent, type AgentType, type Policy, type Service, type ServiceCategory } from "@/lib/policyEngine";
...
export default function AgentsPage() {
  ...
  const policy = loadLocal<Policy | null>("app.policy", null);
  ...
  const contractReady = isConnected && chainId === ACTIVE_CHAIN_ID && hasPaymentGuardConfig() && Boolean(policy?.policyId);
  ...
  async function submitContract(functionName: "approveAgent" | "revokeAgent" | "allowService" | "removeService", target: `0x${string}`) {
    if (!contractReady || !contractAddresses.paymentGuard || !publicClient) {
      throw new Error(`Live mode requires a wallet on ${ACTIVE_NETWORK_NAME}, a current Wave 3 guard address, and an on-chain policy.`);
    }
    const hash = await writeContractAsync({
      ...
      functionName,
      args: [BigInt(policy?.policyId ?? "0"), target]
    });
  ...
  async function addAgent(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setSuccess("Agent permission changed on-chain; the local label was updated for display.");
    } catch (error) {
  ...
  async function addService(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setSuccess("Service permission changed on-chain; the local label was updated for display.");
    } catch (error) {
  ...
  return (
    <main className="page">
      <section className="section">
        ...
        {!contractReady && <div className="rounded-lg border border-amber-400/20 bg-amber-400/10 p-4 text-sm leading-6 text-amber-100">Live guard actions are unavailable. {hasPaymentGuardConfig() ? `The verified Aristotle guard is configured; connect its policy owner wallet on ${ACTIVE_NETWORK_NAME} and load an on-chain policy.` : "Configure NEXT_PUBLIC_PAYMENT_GUARD_ADDRESS with the verified Wave 3 guard."} The legacy Galileo-v1 address is rejected.</div>}
